...
#'                          of the total catch.
#' @param totalCatchVarMethod temporary argument to set the method used to calculate the variance
...
setMethod("sca", signature("FLStock", "FLIndices"),
  ...
  dms <-
    do.call(rbind.data.frame,
            c(list(catch = c(dims(stock), startf = NA, endf = NA)),
              lapply(indices, dims)))
  ...
  if (any(dms$season[-1] > 1))
    stop("only one season per survey - please split into seperate surveys.")
  ...
  for (i in seq(niters)) {
    ...
    if (!missing(covar) & !missing(wkdir)) {
      ...
      outi <- a4aInternal(fmodel = fmodel, qmodel = qmodel, srmodel = srmodel, n1model = n1model,
                          ...
                          useTotalCatchVar = useTotalCatchVar, totalCatchVarMethod = totalCatchVarMethod)
    }
    if (i == 1) {
    ...
    if (fit == "assessment") {
      ...
      if (i == 1) {
        ...
        for (j in seq_along(indices)) {
          ...
          out@pars@qmodel[[j]]@centering[,1] <-
            outi@pars@qmodel[[j]]@centering - outi@pars@stkmodel@centering
        }
      } else {
...
a4aInternal <- function(stock, indices, fmodel = defaultFmod(stock), qmodel = defaultQmod(indices),
  ...
  aux.df <-
    cbind(as.data.frame(stock.n(stock))[c("year", "age")],
          ...
          catch.wt = as.data.frame(catch.wt(stock))$data,
          m.spwn = as.data.frame(m.spwn(stock))$data,
          harvest.spwn = as.data.frame(harvest.spwn(stock))$data)
  ...
  if (sum(a4as) == 0 && max(full.df$year) > max(df.data$year))
    stop("you need to specify a stock recruitment relationship to forecast with out survey information.")
...
#  if (useADMB) { # fit using the ADMB code
    ...
    fitList <- fitADMB(fit, wkdir, df.data, stock, indices, full.df,
                       fbar, plusgroup, useTotalCatchVar, totalCatchVarMethod,
...
#  }
  ...
  index <- lapply(1:length(indices), function(i) {
    ...
    if (is(indices[[i]], 'FLIndexBiomass')) {
      ...
      bb <- apply(nn * zz * stock.wt(stock)[dmns[[1]],
                  dmns[[2]]], 2:6, sum, na.rm = TRUE)
      ii <- qq * bb
    } else {
      nn <- stock.n(a4aout)[dmns[[1]], dmns[[2]]]
      qq <- exp(logq[[i]] - center.log[1] + center.log[i+1])
  ...
  if (fit %in% c("assessment", "MCMC")) {
    ...
    # extract active parameter estimates from returned estimates (out$par.est)
    par_names <- unlist(pnames)
    pars <- unlist(out$par.est[sapply(out$par.std, length) > 0])
    names(pars) <- par_names
    # add names and iter to covariance matrix
    dim(out$cov) <- c(dim(out$cov), 1)
    dimnames(out$cov) <- list(par_names, par_names, 1)

    # build up stock model

    # extract stkmodel parameters and variance covariance matrix
    which_pars <-
      grep("(^fMod:)|(^n1Mod:)|(^rMod:)|(^sraMod:)|(^srbMod:)", par_names)

    # create stkmodel class
    stkmodel <-
      a4aStkParams(
        name = name(a4aout),
        desc = desc(a4aout),
        ...
        n1Mod = n1model,
        srMod = srmodel,
        coefficients = FLPar(pars[which_pars], units = "NA"),
        vcov = out$cov[which_pars, which_pars, 1, drop = FALSE],
        distr = "norm",
        centering = FLPar(centering = center.log[1]),
        link = log,
        linkinv = exp,
        m = m(stock),
        wt = stock.wt(stock),
    ...
    # fill up v submodels
    vmodels <- vector("list", length(indices) + 1)
    which_pars <-
        grep(sprintf("(^vMod:%s:)", fleet.names[1]), par_names)
    vmodels[[1]] <-
        submodel(
          name = fleet.names[1],
          desc = desc(stock),
  ...
  if (fit == "MCMC") {
    ...
    a4aout@pars@stkmodel@coefficients[] <-
      t(out$mcmc$mcmcout[, dimnames(a4aout@pars@stkmodel@coefficients)[[1]]])
    ...
    for (i in fleet.names) {
      a4aout@pars@vmodel[[i]]@coefficients <-
        propagate(a4aout@pars@vmodel[[i]]@coefficients, out$mcmc$nit)
      a4aout@pars@vmodel[[i]]@coefficients[] <-
        t(out$mcmc$mcmcout[, dimnames(a4aout@pars@vmodel[[i]]@coefficients)[[1]]])
      a4aout@pars@vmodel[[i]]@vcov[] <- NA

      if (i != "catch") {
        a4aout@pars@qmodel[[i]]@coefficients <-
          propagate(a4aout@pars@qmodel[[i]]@coefficients, out$mcmc$nit)
        a4aout@pars@qmodel[[i]]@coefficients[] <-
          t(out$mcmc$mcmcout[,dimnames(a4aout@pars@qmodel[[i]]@coefficients)[[1]]])
        a4aout@pars@qmodel[[i]]@vcov[] <- NA
        dmns <- dimnames(a4aout@index[[i]])
        a4aout@index[[i]] <- propagate(a4aout@index[[i]], out$mcmc$nit)
        if (is(indices[[i]], 'FLIndexBiomass')) {
          a4aout@index[[i]][] <- t(out$mcmc$QQ[out$mcmc$idq$s == i &
                                               out$mcmc$idq$y %in% as.numeric(dmns[[2]]),
                                               1, drop = FALSE])
          dmns[[1]] <- ac(srvMinAge[i]:srvMaxAge[i])
    ...
    tmpSumm <-
      with(out,
           c(nopar, NA, NA, nrow(df.data), NA, NA, as.numeric(mcmc$logfile[length(mcmc$logfile) - 2])))
    a4aout@fitSumm <-
      array(tmpSumm,
...
fitADMB <- function(fit, wkdir, df.data, stock, indices, full.df,
  ...
  cat("# Data for the a4a model\n",
      ...
      as.integer(totalCatchVarMethod), "\n",
  ...
  write.t.sparse(Covq, file = filename)
  ...
  pnames <- list(paste0("fMod:",colnames(Xf)),
    paste0("qMod:",
      unlist(
        sapply(
          1:length(indices),
          function(i) paste0(fleet.names[i+1], ":", colnames(Xqlist[[i]]))
        )
      )
    ),
    paste0("vMod:",
      unlist(
        sapply(
          1:length(fleet.names),
          function(i) paste0(fleet.names[i],":",colnames(Xvlist[[i]]))
        )
      )
    ),
    ...
    # read derived model quantities and hessian
      if (!file.exists(paste0(wkdir, '/a4a.cor'))) {
        ...
        out$Q <- array(NA,
                       ...
                       dimnames = list(ages, years, names(indices)))
      } else {
        convergence <- 0
        # read .cor file
