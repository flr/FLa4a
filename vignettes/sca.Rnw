...
<<>>=
n1mod <- ~s(age, k=4)
fit1 <- sca(ple4, ple4.indices[1], n1model=n1mod)
...
<<>>=
fit1 <- sca(ple4, ple4.indices, wkdir="mytest")
@
...
fitmc2 <- sca(ple4, ple4.indices, qmodel=list(~s(age, k=3), ~s(age, k=3),~s(age, k=3)), fit="MCMC", mcmc=mc)
@
...
m4 <- mvrnorm(nits, m4)
range(m4, c("min","max","minyear","maxyear")) <- range(ple4, c("min","max","minyear","maxyear"))
...
lst <- mclapply(split(1:nits, 1:nits), function(x){
	out <- try(sca(iter(stks, x), FLIndices(iter(idxs[[1]], x)), fit="MP"))
